...
C**********************************************************************
      ...
      CHARACTER*80 CTITLE, TTITLE, PTITLE, STITLE
...
C**********************************************************************
     ...
     &  CTITLE,TTITLE,PTITLE,STITLE,MTITLE,VTITLE)
...
C**********************************************************************
     ...
     &  GSJA,GSJB,T0,TREF,TMAX,SMD1,SMD2,SWPEXP,
...
C**********************************************************************
      ...
      REAL RHOSOL(3), PROPP(MAXC), PROPC(MAXC)
	...
	REAL Q10FTABLE(MAXPDATE), Q10WTABLE(MAXPDATE)
      INTEGER DATESN(MAXPDATE), DATESJ(MAXPDATE)
      INTEGER DATESV(MAXPDATE), DATESRD(MAXPDATE)
      INTEGER DATESSLA(MAXPDATE), DATESA(MAXPDATE)
	INTEGER DATESFQ(MAXPDATE), DATESWQ(MAXPDATE)

      CALL READAGEP(UPHY, NOAGEC, NOAGEP)

      CALL READPROP(UPHY, NOAGEC, NOAGEP, PROPC, PROPP)

c      CALL READABSRP(UPHY,NOLAY,ABSRP,REFLEC,TRANS,RHOSOL)
...
C Read in SLA array directly.
     ...
     &  RTEMP, DAYRESP, EFFYRF, TBELOW)

      CALL READRW(UPHY,MODELRW,EFFYRW,RMW,RTEMPW,
...
C**********************************************************************
     ...
     &  THRAB,FCO2,FRESPF,FRESPW,FRESPB, FRESPFr
     &  FH2OT,GSCAN,FH2OCAN,FHEAT)
C Output the hourly totals
C**********************************************************************

      INCLUDE 'MAESTCOM'
      REAL THRAB(KHRS,3),FCO2(KHRS),FRESPF(KHRS),FRESPFr(KHRS)
      REAL FRESPW(KHRS),FRESPB(KHRS)
      REAL GSCAN(KHRS),FH2OT(KHRS),FH2OCAN(KHRS),FHEAT(KHRS)
     ...
     &    FCO2(IHOUR),FRESPF(IHOUR),FRESPFr(IHOUR),FRESPW(IHOUR),
     &    FRESPB(IHOUR),FH2OT(IHOUR)*1e-3,
     &    FH2OCAN(IHOUR)*1E-3,GSCAN(IHOUR),
     &    FHEAT(IHOUR)*1E-3,TCAN(IHOUR)
      END IF
500   FORMAT (I7,1X,2(I4,1X),3(F12.5,1X),8(F12.5,1X))
...
500   FORMAT (I7,1X,I4,1X,9(F12.4,1X))
      END IF
...
C X0,Y0 - if plot is offset from origin. All tree co-ords referenced to this.
...
C STOCKING - no of stems per ha
C**********************************************************************

      INCLUDE 'MAESTCOM'
      NAMELIST /PLOT/ X0,Y0,XMAX,YMAX,NOTREES,
     &  XSLOPE,YSLOPE,BEARING,SHADEHT,PLOTSHAPE
      INTEGER UFILE
...
C BM 7/03 Getting into troble with PROP when NOAGEC = 1 and NOAGEP > 1:
C Fix by having different proportions
C**********************************************************************
...
C**********************************************************************
c      SUBROUTINE READABSRP(UFILE,NOLAY,ABSRP,REFLEC,TRANS,RHOSOLI)
C Read leaf absorptances and reflectances for 3 wavelengths.
C Required input: File unit (UFILE), No of layers (NOLAY).
C**********************************************************************

c      INCLUDE 'MAESTCOM'
c      NAMELIST /ABSORP/ NOLAYERS,RHOSOL,ATAU,ARHO
c      REAL ABSRP(MAXLAY,3),REFLEC(MAXLAY,3),TRANS(MAXLAY,3)
c      REAL ARHO(MAXLAY*3),ATAU(MAXLAY*3)
c      REAL RHOSOL(3),RHOSOLI(3),X(MAXLAY+1)
c      INTEGER UFILE

C Read file
c      REWIND (UFILE)
c      READ (UFILE,ABSORP,IOSTAT = IOERROR)
c      IF (IOERROR.NE.0)
c     &  CALL SUBERROR(
c     &  'ERROR READING REFLECTANCE / TRANSMITTANCE',
c     &  IFATAL,IOERROR)

c      DO 10 IP = 1,NOLAYERS+1
c        X(IP) = (REAL(IP) - 1.0)/REAL(NOLAYERS)
c10    CONTINUE

C Check values & set absorptance array.
c      DO 30 J = 1,3
c        RHOSOLI(J) = RHOSOL(J)
c        DO 20 I = 1,NOLAY
c          D1 = (REAL(I) - 1)/REAL(NOLAY)
c          D2 = (REAL(I))/REAL(NOLAY)
c          REFLEC(I,J) = RINTEG(D1,D2,X,ARHO,(J-1)*NOLAYERS,NOLAY)
c          TRANS(I,J) = RINTEG(D1,D2,X,ATAU,(J-1)*NOLAYERS,NOLAY)
c          ABSRP(I,J) = 1.0 - REFLEC(I,J) - TRANS(I,J)
c20    CONTINUE

c30    CONTINUE

c100   FORMAT(10(1X,F8.3))
...
C**********************************************************************
      ...
      REAL LEAFN(MAXPDATE,MAXLAY,MAXC)
      REAL RDTABLE(MAXPDATE,MAXLAY,MAXC), Q10FTABLE(MAXPDATE)
      INTEGER DATESN(MAXPDATE), DATESRD(MAXPDATE), DATESRFQ(MAXPDATE)
      INTEGER UFILE
...
C**********************************************************************
      ...
      INTEGER UFILE, DATESRWQ(MAXPDATE)
	REAL Q10WTABLE(MAXPDATE)
...
C Used to read in arrays of values specified by date only -
C only foliage and wood Q10's at present
C**********************************************************************

	INCLUDE 'MAESTCOM'
	INTEGER UFILE,DATESI(MAXD)
	REAL RATES(MAXD),RATESI(MAXD)
	CHARACTER*10 DATES(MAXD)
...
C**********************************************************************
     ...
     &  PARAM)
C Same as PHYINTERP but for parameter which is not specified by age & layer
C**********************************************************************

      INCLUDE 'MAESTCOM'
      INTEGER IDATEARR(MAXPDATE)
      REAL PARAMTABLE(MAXPDATE)
...
C BM Jul07 NEWCANOPY is problematic. Change to (a) check change from previous values (not on a daily basis) and 
C (b) put thresholds for different dimensions. 
C**********************************************************************
...
C**********************************************************************
c      SUBROUTINE INTERPOLATET(IDAY, ISTART,
c     &  NOXDATES,DATESX,RXTABLE,
c     &  NOYDATES,DATESY,RYTABLE,
c     &  NOZDATES,DATESZ,RZTABLE,
c     &  NOTDATES,DATEST,ZBCTABLE,
c     &  NODDATES,DATESD,DIAMTABLE,
c     &  NOLADATES,DATESLA,FOLTABLE,TOTLAITABLE,NOTREES,
c     &  RX,RY,RZ,ZBC,FOLT,TOTLAI,DIAM,STOCKING,
c     &  IFLUSH,DT1,DT2,DT3,DT4,EXPTIME,APP,EXPAN,
c     &  NEWCANOPY,CANOPYDIMS)
C Controls the calling of the interpolation routines to get daily values
C of crown heights and radii and leaf area.
C RXTABLE, RYTABLE, etc have all values of dimensions, for all trees and dates
C These are interpolated to give values for this date, for all trees, in RX, RY etc
C**********************************************************************

c      INCLUDE 'MAESTCOM'

c      REAL RX(MAXTT),RY(MAXTT),RZ(MAXTT),ZBC(MAXTT),FOLT(MAXTT)
c      REAL RXTABLE(MAXTDATE,MAXTT),RYTABLE(MAXTDATE,MAXTT)
c      REAL RZTABLE(MAXTDATE,MAXTT),ZBCTABLE(MAXTDATE,MAXTT)
c      REAL FOLTABLE(MAXTDATE,MAXTT),TOTLAITABLE(MAXTDATE)
c      REAL DIAMTABLE(MAXTDATE,MAXTT),DIAM(MAXTT)
c      INTEGER DATESX(MAXTDATE),DATESY(MAXTDATE),DATESZ(MAXTDATE)
c      INTEGER DATEST(MAXTDATE),DATESLA(MAXTDATE),DATESD(MAXTDATE)
c      REAL CANOPYDIMS(6)
c         
C Interpolate to get daily values of crown dimensions
c      CALL TREEINTERP(IDAY,ISTART,NOXDATES,DATESX,RXTABLE,NOTREES,RX)

c      CALL TREEINTERP(IDAY,ISTART,NOYDATES,DATESY,RYTABLE,NOTREES,RY)

c      CALL TREEINTERP(IDAY,ISTART,NOZDATES,DATESZ,RZTABLE,NOTREES,RZ)

c      CALL TREEINTERP(IDAY,ISTART,NOTDATES,DATEST,ZBCTABLE,NOTREES,ZBC)

c      IF (NOLADATES.GT.0) THEN
c        CALL TREEINTERP(IDAY,ISTART,NOLADATES,DATESLA,FOLTABLE,NOTREES,
c     &    FOLT)
c        CALL TREEINTERP(IDAY,ISTART,NOLADATES,DATESLA,TOTLAITABLE,1,
c     &    TOTLAI)
c      ELSE
c        CALL PHENOL(IDAY,ISTART,IFLUSH,DT1,DT2,DT3,DT4,
c     &    EXPTIME,APP,EXPAN,STOCKING,NOTREES,THRESH_FOLT,
c     &    FOLT,TOTLAI)
c      END IF

c      IF (NODDATES.GT.0)
c     &  CALL TREEINTERP(IDAY,ISTART,NODDATES,DATESD,DIAMTABLE,NOTREES,
c     &  DIAM)

C BM 11/07/07 Fixed bug: we were not re-calculating points unless the DAILY change in the canopy
C was large. That allowed small changes to turn into a big change, without POINTS being recalculated.  
C Now, check the change in canopy size since POINTS were last calculated. 
C Thresholds for changes are set here: could potentially set elsewhere to make them easier to change? 
c      NEWCANOPY = 0
c      IF (IDAY.EQ.0) THEN
c        NEWCANOPY = 1
c      ELSE 
c        THRESH_RX = RXTABLE(1,1)*0.01 ! 1% OF INITIAL CROWN RADIUS
c        IF (RX(1)-CANOPYDIMS(1).GT.THRESH_RX) NEWCANOPY = 1
c        THRESH_RY = RYTABLE(1,1)*0.01 
c        IF (RY(1)-CANOPYDIMS(2).GT.THRESH_RY) NEWCANOPY = 1
c        THRESH_RZ = RZTABLE(1,1)*0.01 
c        IF (RZ(1)-CANOPYDIMS(3).GT.THRESH_RZ) NEWCANOPY = 1
c        THRESH_ZBC = ZBCTABLE(1,1)*0.01 ! 
c        IF (ZBC(1)-CANOPYDIMS(4).GT.THRESH_ZBC) NEWCANOPY = 1
c        THRESH_FOLT = 0.05
c        IF (FOLT(1)-CANOPYDIMS(5).GT.THRESH_FOLT) NEWCANOPY = 1
c        IF (FOLT(1)-CANOPYDIMS(5).LT.-THRESH_FOLT) NEWCANOPY = 1
c        THRESH_LAI = 0.1
c        IF (TOTLAI-CANOPYDIMS(6).GT.THRESH_LAI) NEWCANOPY = 1
c        IF (TOTLAI-CANOPYDIMS(6).LT.-THRESH_LAI) NEWCANOPY = 1
c      END IF
c      IF (NEWCANOPY.EQ.1) THEN
c        CANOPYDIMS(1) = RX(1)
c        CANOPYDIMS(2) = RY(1)
c        CANOPYDIMS(3) = RZ(1)
c        CANOPYDIMS(4) = ZBC(1)
c        CANOPYDIMS(5) = FOLT(1)
c        CANOPYDIMS(6) = TOTLAI
c      END IF
