...
C**********************************************************************
     ...
     +  G0,D0L,GAMMA,G1,
     +  WLEAF,NSIDES,ITERMAX,
     +  GSC,ALEAF,RD,ET,FHEAT,TLEAF,GBH
     +  )
...
C      FHEAT = (TLEAF - TAIR)*2.*GBH*CPAIR*AIRMA  !BM 12/05 Not correct - use energy bal instead
...
C**********************************************************************
	...
	REAL GSJA,GSJB,T0,TREF,TMAX
      ...
      REAL GS,ALEAF,RD
      INTEGER MODELGS,IQERROR,IECO,SOILDATA
...
C**********************************************************************
	...
	IF (GV.GT.0.0) THEN
     ...
     &		(SLOPE + GAMMA * GH/GV)
      ...
      END IF
      PENMON = ET / LHV
C	IF (PENMON.LT.0.0) PENMON = 0.0		! BM 12/05 Should not be negative
...
C**********************************************************************
      FUNCTION CALCFSOIL(SOILMOIST,SOILDATA,SMD1,SMD2,SWPEXP)
C Calculate the effect of soil water content on stomatal conductance
C Two alternative forms:
C Granier & Loustau 1994 Fs = 1-a exp(b SMD) where SMD is soil moisture deficit, dimnless
C Otherwise negative exponential function of soil water potential
C**********************************************************************
...
C Exponential relationship with potential: parameter = SWPEXP
      IF (SOILDATA.EQ.POTENTIAL) THEN
	  IF (SWPEXP.GT.0.0) THEN
	    CALCFSOIL = EXP(SWPEXP*SOILMOIST)
	  END IF
C Exponential relationship with deficit: params SMD1,SMD2 
      ELSE IF (SOILDATA.EQ.DEFICIT) THEN
        IF (SMD1.GT.0.0) THEN
	    CALCFSOIL = 1.0 - SMD1*EXP(SMD2*SOILMOIST)
C Linear decline with increasing deficit: pUT SMD1 < 0, param SMD2
