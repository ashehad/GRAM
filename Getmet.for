...
C SATUR - calculate water vapour saturation pressure at given T MOVED TO UTILS
C TK - calculate T in Kelvin given T in Celsius MOVED TO UTILS
...
C   SOILMOIST - soil moisture data - either potential (MPa) or deficit (dimnless)
C   SOILDATA - type of soil moisture data
C**********************************************************************
...
C**********************************************************************
      ...
      REAL ZEN(KHRS)
      REAL DATAIN(MAXMET)
      REAL windah(KHRS),tsoil(KHRS),tair(KHRS),PPT(KHRS),SOILMOIST(KHRS)
...
C Calculate soil moisture deficit - assume unchanging over course of day
	...
	ELSE IF (METCOLS(MDSW).NE.MISSING) THEN
	  SOILDATA = DEFICIT
...
C**********************************************************************
      ...
      REAL CA(KHRS),PRESS(KHRS),ZEN(KHRS),PPT(KHRS),SOILMOIST(KHRS)
	REAL DELTAT(12)
      INTEGER METCOLS(MAXMET),SOILDATA
...
C Get sOil moisture - either soil water potential or soil moisture deficit
      DO 170 IHR = 1,KHRS
        SOILMOIST(IHR) = 0.0
170   CONTINUE   
      SOILDATA = NONE   
...
c	USE MSFLIB        ! Library required by COMPAQ VISUAL FORTRAN - superseded
...
c      CALL SEED(1995) ! For testing - same random numbers - change to 0 later. 
      IFLAG = 0
C 1. All rain falls in one hour for light storms (<2 mm)
      IF (TOTAL.LE.2.) THEN
        RANVAL = RANDOM(IFLAG)   ! randomly select hour
...
C 3. All rain falls at 2mm/hour at a random time of the day */
	  ...
	  DO 20 I = 1, IHRSWITHRAIN
          RANVAL = RANDOM(IFLAG)   ! randomly select hours
	    IRAIN = INT(RANVAL*KHRS)+1
...
C**********************************************************************
      ...
      DO 10 I = 1,KHRS
C Brutsaert et al formula
C        EA = SATUR(TAIR(I)) - VPD(I) !Old formula - see comments
C        EMSKY = 0.642*(EA/TK(TAIR(I)))**(1./7.)
C        RADABV(I,3) = EMSKY*SIGMA*(TK(TAIR(I))**4)

C Monteith and Unsworth formula
C	  SIGT4 = SIGMA*(TK(TAIR(I))**4)
C	  EMCLEAR = 1.06*SIGT4 - 119.
C	  RADABV(I,3) = FSUN(I)*EMCLEAR + (1.-FSUN(I))*(0.84+0.16*EMCLEAR)

C BM 12/05: Am hybridising. Use Brutsaert et al for clear sky emission but
C Monteith and Unsworth correction for cloudy sky. 
C The 1.06 SIGT4 - 119 formula seems problematic in some conditions. 
C Also suspect I had correction incorrect - check this!!
