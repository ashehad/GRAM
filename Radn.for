...
C NOAGEP: no of age classes for which physiological params specified
...
C FOLLAY: the amount of foliage in each layer
C CANOPYDIMS: canopy dimensions when these gridpoints were calculated
C**********************************************************************
...
C BM 7/03: Moved this within loop to correct error with JLEAF = 0, NOAGEC > 1, NOAGEP > 1
        DO 100 IPT = 1,NUMPNT
          DLT(IPT) = 0.0
          DO 60 IAGE = 1,NOAGEC
...
c BM Mar07 If the shape is a cylinder, you want the upper point of intersection
C Otherwise, you want the point that is neither above nor below the tree crown. 
      IF (ISITU.EQ.1) THEN
        X1 = XX
        Y1 = YY
        Z1 = ZZ
...
C BM ADDED 27/3/07
C If the upper point is above the top of the crown, take the
C point of intersection with the top plane as the upper point.
C This can only happen if you're using the cylinder shape. 
      IF (Z2.GT. (ZBCQ+DZTQ+RZQ)) THEN
         Z2 = ZBCQ + DZTQ + RZQ
...
C Corrected 07/05 BM
C Code from ORIGMAES from Ying Ping
C Note: BEAR is the bearing (in degrees clockwise) of the x-axis from south
C If the x-axis is S, BEAR = 0; x-axis W, BEAR = 90; x-axis N, BEAR 180; x-axis E, BEAR -90
C AZ, the azimuth angle, is in radians anticlockwise from S (trig convention) 
C e.g. azimuth S, AZ = 0; azimuth W, AZ = -pi/2; azimuth N, AZ = pi, azimuth E, AZ = +pi/2
C**********************************************************************
...
C PREVIOUS INCORRECT CODE
C        AZ(I) = Azimth-BEAR
C        IF (ALAT.LT.0.0) AZ(I)=-AZ(I)
